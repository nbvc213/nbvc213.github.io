...
其次，子类继承父类后，并不是父类中的所有内容都能直接拿来用。一般来说，**构造方法不能被继承，父类中被 `private` 修饰的成员也不能被子类直接访问**；除此之外，其他符合访问规则的成员都可以被继承。
...
【单选题】关于 Java 中“继承”的理解，下列说法正确的是哪一项？

- A. 继承的本质是把父类代码原样复制到子类中，因此子类和父类彼此独立
- B. 继承最基本的价值是代码复用，更重要的价值是为后续的方法重写和多态提供基础
- C. 只要两个类之间有联系，就都适合使用继承
- D. 子类继承父类后，父类中所有成员都可以被子类直接访问

【单选题】关于 Java 继承的基本规则，下列说法正确的是哪一项？

- A. Java 支持一个类同时直接继承多个父类，只要这些父类中没有同名成员即可
- B. Java 不支持多重继承，因为多层继承会导致所有祖先类成员全部失效
- C. Java 只支持单继承，但支持多重继承；同时，构造方法不能被继承，`private` 成员也不能被子类直接访问
- D. 只要父类中定义了构造方法，子类就一定会自动继承这些构造方法

【单选题】阅读下面代码，关于该程序的说法正确的是哪一项？
...
class AirConditioner extends Appliance {
    ...
    public void cool() {
        System.out.println("当前制冷温度：" + temperature);
    }
}
...
- A. 程序编译失败，因为 `AirConditioner` 只能使用自己声明的成员，不能直接使用父类中的 `brand` 和 `powerOn()`
- B. 程序可以正常运行，说明子类对象既可以使用继承来的非私有成员，也可以使用自己特有的成员
- C. 程序编译失败，因为子类必须把父类中的属性和方法重新声明一遍才能使用
- D. 程序可以正常运行，但 `brand` 会自动变成 `private`，因此 `powerOn()` 实际输出的是 `null`
...
public class CouponTest {
    public static void main(String[] args) {
        Coupon c = new Coupon();
        System.out.println(c.toString() != null);
        System.out.println(c.hashCode() >= 0 || c.hashCode() < 0);
    }
}
```
...
- B。继承表示在已有类的基础上继续扩展出新类。它最基本的价值是**代码复用**，更重要的价值是为后续的**方法重写和多态**提供基础。A 错在把继承理解成“原样复制”，更准确的说法应当是“子类在父类基础上继续扩展”；C 错在忽略了继承强调的是 **is-a** 关系，不是“有联系就继承”；D 错在父类成员是否能被子类直接访问，还要看访问权限，尤其 `private` 成员不能被子类直接访问。
- C。Java 的类继承只支持**单继承**，不支持一个类同时直接继承多个类；但支持**多重继承**，也就是多层继承。同时，这一节明确强调：**构造方法不能被继承，父类中被 `private` 修饰的成员也不能被子类直接访问。**A 错在 Java 不支持多继承；B 错在 Java 恰恰支持多重继承；D 错在构造方法本来就不能被继承。
- B。`AirConditioner extends Appliance` 说明空调类继承了家电类，因此空调对象既可以使用从父类继承来的 `brand` 和 `powerOn()`，也可以使用自己特有的 `temperature` 和 `cool()`。这道题考查的关键是：**子类对象可以直接使用父类中可继承的非私有成员，同时保留自己的特有成员。**A、C 都错在误以为子类必须重新声明父类成员；D 错在 `brand` 并不会因为继承自动变成 `private`。
- A。即使 `Coupon` 类中什么都没有显式定义，它仍然**默认继承 `java.lang.Object`**，因此 `toString()`、`hashCode()` 等方法依然可以调用。`c.toString() != null` 的结果是 `true`；而 `c.hashCode()` 返回的是一个 `int` 值，不管这个值是正数、负数还是零，表达式 `>= 0 || < 0` 总会成立，所以第二行也是 `true`。B、C 都只对了一半；D 错在这段代码完全可以正常编译运行。
- A。第一次 `new PremiumAccount()` 时，类会被首次主动使用，因此先加载父类，再加载子类；静态相关内容只执行一次，且**先父类静态，后子类静态**。随后开始创建对象，并按**先父类对象初始化与构造，后子类对象初始化与构造**的顺序执行。第二次再创建同一个子类对象时，类已经加载过了，所以不会再执行静态代码块，只会重复对象创建阶段。因此 A 的顺序完全正确。B 错在把第一次对象创建时的静态与对象初始化顺序写乱了，而且第二次还错误地重复执行了静态代码块；C 把父类和子类的先后顺序整体颠倒了；D 错在代码语法没有问题。

【实操题】请使用 **IDEA** 完成本题。创建一个 Java 项目，项目名为 `oop-inheritance-practice`，在 `src` 下创建包 `com.homework.oop01`，并在该包中创建完成本题所需的类和测试程序。

某学校需要管理校内不同类型人员的信息。学生和教师都属于“人”，都具有姓名、年龄，也都可以进行自我介绍；但学生有自己的学习行为，教师有自己的授课行为。请使用**继承**完成程序设计，将学生和教师共有的属性和行为提取到父类中，避免重复编写相同代码。

程序运行时需要完成以下效果：

1. 创建一个学生对象，设置姓名为 `张三`，年龄为 `18`；
2. 创建一个教师对象，设置姓名为 `李老师`，年龄为 `35`；
3. 学生对象先输出自我介绍，再输出学习行为；
4. 教师对象先输出自我介绍，再输出授课行为；
5. 代码中应体现“学生是人”“教师是人”的继承关系，而不是把公共属性和公共方法分别重复写在两个类中。
...
public class Teacher extends Person {
    public void teach() {
        System.out.println(name + "正在讲授面向对象编程。");
    }
}
```
...
【单选题】关于“方法覆盖”的理解，下列说法正确的是哪一项？

- A. 方法覆盖指的是子类把父类中的属性也全部重新声明一遍
- B. 方法覆盖指的是子类继承父类后，对父类原有的实例方法重新实现；本质上改的是方法体中的具体行为，而不是随意改方法结构
- C. 只要两个类中出现了同名方法，就一定构成方法覆盖
- D. 方法覆盖发生后，父类中的原方法会从类中被删除
...
- A. 程序编译失败，因为重写方法时返回值类型必须绝对完全一致
- B. 程序编译成功，因为子类返回值类型可以与父类无关，只要方法名一样即可
- C. 程序编译成功，因为子类方法与父类方法方法名、参数列表一致，且 `String` 是 `Object` 的子类
- D. 程序编译失败，因为只要加了 `@Override`，返回值类型就不能改变
...
class EmailService extends MessageService {
    @Override
    public void snd(String content) {
        System.out.println("发送邮件：" + content);
    }
}
```

- A. 程序编译成功，`snd` 只是 `send` 的另一种简写写法
- B. 程序编译成功，因为加不加 `@Override` 都不影响编译阶段检查
- C. 程序编译失败，因为 `snd` 并没有真正重写父类方法，`@Override` 会在编译阶段帮助检查这一点
- D. 程序编译失败，因为子类方法体中输出语句与父类不同

【单选题】关于方法覆盖时的访问权限和异常规则，下列说法正确的是哪一项？

- A. 父类方法是 `protected`，子类重写时改成 `private` 是允许的，因为这是子类自己的实现
- B. 父类方法声明 `throws Exception`，子类重写时声明 `throws Throwable` 是允许的，因为 `Throwable` 更通用
- C. 父类方法是 `protected`，子类重写时改成 `public` 是允许的；父类方法声明了编译时异常，子类重写时可以不抛，或抛得更少、更窄
- D. 子类重写父类方法时，访问权限和异常声明都必须与父类完全一致，不能有任何变化
...
public class ReportTest {
    public static void main(String[] args) {
        BaseReport r1 = new BaseReport();
        BaseReport r2 = new SalesReport();
        SalesReport r3 = new SalesReport();
...
- B。方法覆盖，也叫方法重写，指的是子类继承父类之后，对父类原有的**实例方法**进行重新实现。它解决的是“父类已经给出了方法入口，但不同子类执行方式不同”的问题，所以本质上改的是**实现**，不是随意改方法结构。A 把方法覆盖误解成属性重声明；C 错在“同名方法”并不必然构成重写，还必须满足继承关系、方法名一致、参数列表一致等条件；D 错在父类原方法并不会消失，子类只是提供了自己的实现版本。
- C。构成方法覆盖的核心条件之一，是父类中先有这个方法，且子类方法与父类方法**方法名相同、形式参数列表相同**。在返回值方面，初学阶段常记为“应当一致”，更准确地说，**返回值类型相同，或者是父类返回值类型的子类，也可以构成重写**。这里父类返回 `Object`，子类返回 `String`，而 `String` 正是 `Object` 的子类，所以该写法合法。A、D 都错在把返回值规则理解得过死；B 错在返回值不是“随便什么类型都行”。
- C。`@Override` 的作用主要是让编译器在**编译阶段**帮助检查：当前方法是否真的重写了父类方法。这里父类方法名是 `send`，子类却写成了 `snd`，因此它并没有真正重写父类方法。一旦加上 `@Override`，编译器就会直接报错。A 错在 `snd` 不是 Java 认可的“简写”；B 错在 `@Override` 恰恰会触发编译阶段检查；D 错在方法体不同本来就是重写时允许存在的现象，真正的问题出在方法头不匹配。
- C。方法覆盖有两条非常容易混淆的规则。第一，**访问权限不能更低，可以相同，也可以更高**，因此父类是 `protected` 时，子类改成 `public` 是允许的，但改成 `private` 不允许。第二，父类方法如果声明了编译时异常，子类重写时**不能抛出比父类更多、更宽的编译时异常**，但可以不抛，或者抛得更少、更窄。A 错在把权限缩小了；B 错在 `Throwable` 比 `Exception` 更宽；D 错在规则并不是“必须完全一样”。
- B。这道题考查的是“**静态方法不存在真正意义上的方法覆盖**”。静态方法属于类，不属于对象，因此这里不是动态绑定，而是**方法隐藏**。调用哪个静态方法，看的是**引用的编译时类型或类名本身**。`r1` 的类型是 `BaseReport`，输出“基础报表”；`r2` 虽然实际指向 `SalesReport` 对象，但引用类型仍是 `BaseReport`，所以依然输出“基础报表”；`r3` 的类型就是 `SalesReport`，因此输出“销售报表”。A、C 都错在把静态方法误当成了实例方法那样的动态绑定；D 错在代码语法没有问题。
- C。这道题同时考查“实例变量”和“实例方法”在继承场景下的不同规则。`u.role` 访问的是**实例变量**，变量访问看的是**引用的编译时类型**，因此这里看到的是 `User` 中的 `role`，输出“普通用户”。而 `u.showRole()` 调用的是**实例方法**，`Admin` 对它进行了重写，因此运行时会根据对象的实际类型执行子类版本，输出“子类方法：管理员”。A 错在把方法调用也按变量规则理解了；B 错在把变量访问也按方法动态绑定理解了；D 两部分都判断错了。

【实操题】请使用 **IDEA** 完成本题。自行创建一个 Java 项目，项目名为 `oop-override-practice`，在 `src` 下创建包 `com.homework.oop02`，并在该包中创建完成本题所需的类和测试程序。

某消息提醒系统需要向用户发送通知。系统中有一种通用通知，默认输出普通提醒内容；同时还有短信通知和邮件通知，它们都属于通知，但具体提醒方式不同。请使用**继承和方法覆盖**完成程序设计，让不同子类重新实现父类中已有的通知方法。

程序运行时需按以下要求输出：

1. 创建一个通用通知对象，调用通知方法后输出 `系统通知：您有一条新消息。`；
2. 创建一个短信通知对象，调用通知方法后输出 `短信通知：验证码为 9527，请及时查收。`；
3. 创建一个邮件通知对象，调用通知方法后输出 `邮件通知：您的账单已发送至邮箱。`；
4. `SmsNotice` 和 `EmailNotice` 必须继承同一个父类；
5. 子类中必须使用 `@Override` 标记被重写的方法；
6. 子类重写的方法必须与父类方法保持相同的方法名和参数列表，不能通过新写一个相似方法来代替方法覆盖。
...
```java
package com.homework.oop02;

public class Notice {
    public void send() {
        System.out.println("系统通知：您有一条新消息。");
    }
}
```
...
public class SmsNotice extends Notice {
    @Override
    public void send() {
        System.out.println("短信通知：验证码为 9527，请及时查收。");
    }
}
```
...
public class EmailNotice extends Notice {
    @Override
    public void send() {
        System.out.println("邮件通知：您的账单已发送至邮箱。");
    }
}
```
...
【单选题】关于 Java 中多态的理解，下列说法正确的是哪一项？

- A. 只要两个类中有同名方法，就一定可以形成多态
- B. 多态通常建立在继承和方法覆盖的基础上，常见形式是父类型引用指向子类对象
- C. 多态发生后，父类型引用可以直接调用子类中特有的所有方法
- D. 多态只影响实例变量，不影响实例方法的调用结果

【单选题】阅读下面代码，程序的输出结果是哪一项？
...
class VipAccount extends Account {
    ...
    @Override
    public void showInfo() {
        System.out.println("VIP账户信息");
    }

    public void useVipService() {
        System.out.println("使用VIP服务");
    }
}

public class Test {
    public static void main(String[] args) {
        Account account = new VipAccount();
...
- A. 程序可以正常运行，依次输出 `猫在移动` 和 `猫抓老鼠`
- B. 程序可以正常运行，依次输出 `动物在移动` 和 `猫抓老鼠`
- C. 程序编译错误，因为 `Animal` 类型的引用不能直接调用 `Cat` 类中特有的 `catchMouse()` 方法
- D. 程序运行时报错，因为 `Animal animal = new Cat();` 属于非法写法

【单选题】阅读下面代码，程序的运行结果是哪一项？
...
- B. `鸟儿唱歌`
- C. 编译错误
- D. 运行时报 `ClassCastException`

【答案】

- B。多态不是简单的“方法同名”，而是通常建立在继承和方法覆盖的基础上。最常见的写法是父类型引用指向子类对象，例如 `Animal a = new Cat();`。此时编译阶段看左边的父类型，运行阶段执行的实例方法可能来自右边实际创建的子类对象。A 错在忽略了继承关系和方法覆盖；C 错在父类型引用只能直接调用父类中声明过的成员，不能直接调用子类特有方法；D 错在多态主要影响的是实例方法调用，不是实例变量访问。

- C。`account.type` 访问的是实例变量，实例变量不参与多态，访问时看引用变量的编译时类型。这里 `account` 的类型是 `Account`，所以输出 `普通账户`。`account.showInfo()` 调用的是实例方法，并且子类 `VipAccount` 重写了该方法，因此运行时根据对象的实际类型执行子类版本，输出 `VIP账户信息`。A 错在忽略了方法覆盖后的动态绑定；B 错在把实例变量也当成了多态；D 错在把实例变量和实例方法的规则都判断反了。

- C。`Animal animal = new Cat();` 是合法的向上转型，表示把子类对象当作父类型对象来看待。`animal.move()` 可以调用，因为 `move()` 在父类 `Animal` 中已经声明，并且运行时会执行子类重写后的版本。但是 `catchMouse()` 是 `Cat` 类中特有的方法，父类 `Animal` 中没有这个方法。编译器只看 `animal` 的编译时类型是 `Animal`，因此不允许直接调用 `catchMouse()`。如果确实要调用子类特有方法，需要先进行安全的向下转型。

- B。`animal` 的编译时类型是 `Animal`，但它实际指向的是 `new Bird()` 创建出来的 `Bird` 对象。第一次判断 `animal instanceof Cat` 的结果为 `false`，不会进入该分支，因此不会发生错误的强制转换。第二次判断 `animal instanceof Bird` 的结果为 `true`，于是将其向下转型为 `Bird` 后调用 `sing()`，最终输出 `鸟儿唱歌`。D 错在本题已经使用 `instanceof` 避免了错误转型；如果直接把 `new Bird()` 得到的对象强制转成 `Cat`，才会在运行时产生 `ClassCastException`。

【实操题】请使用 **IDEA** 完成本题。自行创建一个 Java 项目，项目名为 `oop-polymorphism-practice`，在 `src` 下创建包 `com.homework.polymorphism01`，并在该包中创建完成本题所需的类和测试程序。

某门店有不同类型的配送工具，例如自行车和汽车。它们都属于配送工具，都可以执行“配送”行为，但不同工具的配送方式不同。同时，门店希望有一个统一的调度方法，能够接收任意一种配送工具对象，并根据对象的实际类型执行对应的配送行为。

请使用**继承、方法覆盖和多态**完成程序设计。

程序运行时需按以下要求输出：

1. 定义一个父类表示配送工具，父类中包含配送工具名称，并提供一个配送方法；
2. 定义自行车类和汽车类，让它们继承配送工具类，并重写配送方法；
3. 在测试类中编写一个静态方法，用父类类型作为形参，统一接收不同子类对象；
4. 在 `main` 方法中分别创建自行车对象和汽车对象，并使用父类类型引用接收它们；
5. 分别调用统一调度方法，使同一个方法在不同对象上传递时表现出不同配送效果；
6. 输出结果应体现：同一个父类类型的参数，接收不同子类对象时，执行的是不同子类重写后的方法。
...
```java
package com.homework.polymorphism01;

public class DeliveryTool {
    String name;

    public void deliver(String goods) {
        System.out.println(name + "正在配送：" + goods);
    }
}
```
...
public class Bike extends DeliveryTool {
    @Override
    public void deliver(String goods) {
        System.out.println(name + "通过自行车配送：" + goods);
    }
}
```
...
public class Car extends DeliveryTool {
    @Override
    public void deliver(String goods) {
        System.out.println(name + "通过汽车配送：" + goods);
    }
}
```
...
public class DeliveryTest {
    public static void main(String[] args) {
        DeliveryTool tool1 = new Bike();
        tool1.name = "一号配送员";

        DeliveryTool tool2 = new Car();
        tool2.name = "二号配送员";
    ...
    public static void dispatch(DeliveryTool tool, String goods) {
        System.out.println("开始调度配送任务");
        tool.deliver(goods);
    }
}
```
...
【单选题】关于 Java 中的 `super` 关键字，下列说法正确的是哪一项？

- A. `super` 是一个独立的父类对象引用，因此可以直接使用 `System.out.println(super)`
- B. `super` 表示当前对象中父类那部分特征，可以用于访问父类成员或调用父类构造方法
- C. 每次执行 `super()`，都会在堆内存中额外创建一个父类对象
- D. `super` 可以在任何普通类中使用，与当前类是否存在父类无关
...
- A. 程序正常运行，依次输出 `创建商品：null` 和 `创建手机`
- B. 程序正常运行，只输出 `创建手机`
- C. 程序编译错误，因为 `Phone()` 会默认调用 `super()`，但父类中没有无参数构造方法
- D. 程序运行时报错，因为父类构造方法必须由测试类直接调用
...
class Vehicle {
    public Vehicle(int number) {
        System.out.println("车辆编号：" + number);
    }
}
```
...
- B。`super` 不是一个可以单独保存和输出的引用变量，而是一个用于访问当前对象中父类部分的关键字。在子类中，可以通过 `super.成员变量` 访问父类中的同名变量，通过 `super.方法名()` 调用父类原有方法，也可以通过 `super(实参)` 调用父类构造方法。A 错在 `super` 不能单独作为表达式使用；C 错在 `super()` 不会额外创建父类对象，它只是初始化当前子类对象中的父类部分；D 错在 `super` 用于继承场景，必须结合当前类的父类部分理解。

- A。执行 `role` 和 `this.role` 时，都会按照就近原则访问子类 `Manager` 中定义的 `role`，因此前两行均输出 `部门经理`。`super.role` 明确指定访问父类 `Employee` 中的同名变量，因此第三行输出 `普通员工`。

  `work()` 和 `this.work()` 都表示调用当前对象的实例方法。由于 `Manager` 已经重写了 `work()`，所以这两次调用都执行子类版本，输出 `安排部门工作`。`super.work()` 明确绕过子类重写后的方法，调用父类原来的实现，因此最后输出 `完成基础工作`。

  B 错在把 `super.role` 和 `super.work()` 也当成了子类成员；C 错在没有按照就近原则和方法重写规则判断；D 错在代码语法正确，可以正常编译运行。

- C。`Phone` 的构造方法第一行没有显式编写 `this(...)` 或 `super(...)`，编译器会自动补上无参数的 `super()`，相当于：
...
```
  ...
  但是父类 `Product` 已经手动定义了一个有参数构造方法，因此系统不会再为它提供默认的无参数构造方法。父类中不存在 `Product()`，所以子类构造方法中自动补出的 `super()` 找不到对应构造方法，程序在编译阶段就会报错。

  A 错在 Java 不会自动把缺少的构造参数补成 `null`；B 错在创建子类对象之前必须先完成父类部分的初始化；D 错在父类构造方法通常由子类构造方法通过 `super(...)` 调用，不需要测试类直接调用。

- C。`this(101)` 位于无参数构造方法的第一行，用于调用本类中的 `Bus(int number)` 构造方法；进入有参数构造方法后，第一行再通过 `super(number)` 调用父类的有参数构造方法。整个调用链最终能够完成父类部分和子类部分的初始化，因此 C 可以正常编译。

  A 错在 `super(101)` 必须位于构造方法第一行，前面不能出现其他执行语句。B 错在 `super(...)` 和 `this(...)` 都要求位于构造方法第一行，同一个构造方法中不能同时直接出现二者。D 错在 `super()` 试图调用父类无参数构造方法，但父类只提供了 `Vehicle(int number)`。

【实操题】请使用 **IDEA** 完成本题。自行创建一个 Java 项目，项目名为 `oop-super-practice`，在 `src` 下创建包 `com.homework.oop03`，并在该包中创建完成本题所需的类和测试程序。

某网上书店需要管理普通图书和进口图书。所有图书都有书名和价格，并且都可以输出自己的基础信息；进口图书除了具有这些基础信息外，还需要记录来源国家和进口税率。

请根据需求使用继承和 `super` 关键字完成程序设计。

程序运行时需满足以下要求：

1. 将图书共有的书名和价格定义在父类中，并使用 `private` 修饰；
2. 父类只提供能够接收书名和价格的有参数构造方法，不提供无参数构造方法；
3. 进口图书类继承图书类，并增加来源国家和进口税率；
4. 创建进口图书对象时，必须通过 `super(实参)` 完成父类中书名和价格的初始化；
5. 进口图书重新实现基础信息输出行为时，应先输出图书类型，再通过 `super.方法名()` 复用父类已有的输出逻辑，最后输出进口图书特有的信息；
6. 在测试类中创建一本书名为 `Java 核心技术`、价格为 `129.0` 元、来源国家为 `美国`、进口税率为 `8.0%` 的进口图书，并输出完整信息。
...
public class Book {
    ...
    public void printInfo() {
        ...
        System.out.println("图书价格：" + price + "元");
    }
}
```
...
public class ImportedBook extends Book {
    ...
    public ImportedBook(String name, double price,
                        String originCountry, double taxRate) {
        super(name, price);
    ...
    @Override
    public void printInfo() {
        System.out.println("图书类型：进口图书");
        super.printInfo();
        System.out.println("来源国家：" + originCountry);
        System.out.println("进口税率：" + taxRate + "%");
    }
}
```
...
public class BookTest {
    public static void main(String[] args) {
        ImportedBook book =
                new ImportedBook("Java 核心技术", 129.0, "美国", 8.0);
...
【单选题】关于 Java 中 `final` 关键字的使用，下列说法正确的是哪一项？

- A. 被 `final` 修饰的类不能创建对象，只能被其他类继承
- B. 被 `final` 修饰的方法不能被子类继承，也不能被子类调用
- C. 被 `final` 修饰的变量只能赋值一次，但不一定必须在声明变量时立即赋值
- D. 被 `final` 修饰的引用变量指向对象后，该对象的所有属性都不能再发生变化
...
- C。`final` 修饰变量时，限制的是变量只能完成一次赋值，而不是要求变量必须在声明时立即赋值。例如，局部变量可以先声明为 `final`，再在后面的语句中完成第一次赋值；但一旦完成赋值，就不能再次赋值。

  A 错在 `final` 修饰类表示该类不能被继承，但仍然可以正常创建对象。B 错在 `final` 方法可以被子类继承和调用，只是不能被子类重新实现。D 错在 `final` 修饰引用变量时，固定的是引用变量中保存的对象地址，不能让该引用重新指向另一个对象；如果对象本身的属性允许修改，仍然可以通过该引用修改对象内部的数据。

- C。`order` 是一个被 `final` 修饰的引用变量，它在接收 `new Order()` 创建的对象后，不能再被赋值为另一个对象引用。但是，`final` 并不会自动把 `Order` 对象变成不可修改的对象。

  `order.status = "已支付";` 和 `order.status = "已发货";` 修改的是对象内部的 `status` 属性，并没有改变 `order` 保存的对象引用，因此两条语句都可以正常执行。最后 `status` 的值为 `"已发货"`，所以输出 `已发货`。
...
```
  ...
  这条语句试图让 `order` 重新指向另一个对象，违反了 `final` 引用变量只能赋值一次的规则。

【实操题】请使用 **IDEA** 完成本题。自行创建一个 Java 项目，项目名为 `oop-final-practice`，在 `src` 下创建包 `com.homework.finalpractice01`，并在该包中创建完成本题所需的类和测试程序。

某客运平台需要管理电子车票。每张车票在创建时都会确定一个唯一票号，车票创建完成后，票号不能再被修改；车票状态则可以随着业务过程发生变化，例如从“待使用”变为“已使用”。

系统还需要保存统一的平台名称，该名称属于整个车票系统，不属于某一张具体车票，并且在程序运行期间不能被修改。车票中用于输出票号的方法属于固定的核心功能，任何子类都可以继承和使用，但不允许重新实现。

电子车票属于车票的一种，除票号和状态外，还需要记录乘车人的姓名，并能够输出完整的电子车票信息。

请根据需求使用继承和 `final` 关键字完成程序设计。

程序运行时需满足以下要求：

1. 平台名称固定为 `城市客运平台`，应作为类级别的常量保存；
2. 车票票号必须在创建对象时完成初始化，之后不能再次修改；
3. 输出票号的方法必须使用 `final` 修饰，防止子类重新实现；
4. 创建一张票号为 `T2026001`、乘车人为 `张三`、初始状态为 `待使用` 的电子车票；
5. 使用 `final` 修饰保存该电子车票对象的引用变量；
6. 先输出车票的初始完整信息，再将车票状态修改为 `已使用`，然后重新输出当前状态；
7. 程序应体现：`final` 引用变量不能重新指向另一个对象，但仍然可以修改原对象中允许变化的属性。
...
```java
package com.homework.finalpractice01;

public class Ticket {
    public static final String PLATFORM_NAME = "城市客运平台";
    ...
    public final void printTicketNumber() {
        System.out.println("车票票号：" + ticketNumber);
    }
...
`ElectronicTicket.java`

```java
package com.homework.finalpractice01;

public class ElectronicTicket extends Ticket {
    private String passengerName;

    public ElectronicTicket(String ticketNumber, String status,
                            String passengerName) {
        super(ticketNumber, status);
        this.passengerName = passengerName;
    }

    public void printInfo() {
        System.out.println("平台名称：" + PLATFORM_NAME);
        printTicketNumber();
        System.out.println("乘车人：" + passengerName);
        System.out.println("车票状态：" + getStatus());
    }

    /*
    printTicketNumber() 已被父类使用 final 修饰，
    子类不能重新实现该方法。
...
public class TicketTest {
    public static void main(String[] args) {
        final ElectronicTicket ticket =
                new ElectronicTicket("T2026001", "待使用", "张三");

        ticket.printInfo();

        System.out.println("状态修改后：");

        ticket.changeStatus("已使用");
        System.out.println("车票状态：" + ticket.getStatus());

        /*
        ticket = new ElectronicTicket(
                "T2026002", "待使用", "李四"
        );

        ticket 是 final 引用变量，不能重新指向另一个对象。
        */
    }
}
```
...
【单选题】关于 Java 中抽象类和抽象方法的关系，下列说法正确的是哪一项？

- A. 只要一个类被 `abstract` 修饰，其中就必须至少定义一个抽象方法
- B. 抽象方法可以定义在普通类中，只要该方法不会被直接调用即可
- C. 抽象类中可以没有抽象方法，但包含抽象方法的类必须声明为抽象类
- D. 抽象类不能定义普通方法，因为其中的所有方法都必须交给子类实现

【单选题】下列代码中，可以正常编译的是哪一项？
...
abstract class Notice {
    public Notice() {
        System.out.println("Notice 构造方法");
    }

    public void prepare() {
        System.out.println("准备通知内容");
    }
...
class EmailNotice extends Notice {
    public EmailNotice() {
        System.out.println("EmailNotice 构造方法");
    }
...
- D. 编译错误，因为抽象类不能定义构造方法，也不能作为引用变量的类型

【答案】

- C。抽象类中既可以包含抽象方法，也可以完全不包含抽象方法。只要设计者希望某个父类只作为公共模板、不允许被直接创建对象，就可以把它声明为抽象类。反过来，抽象方法没有方法体，属于尚未完成的行为，因此只要一个类中包含抽象方法，该类就必须使用 `abstract` 修饰。

  A 错在抽象类不一定包含抽象方法；B 错在普通类中不能声明抽象方法；D 错在抽象类不仅可以包含普通方法，还可以包含成员变量、构造方法以及其他已经实现的成员。

- C。C 中的 `Animal` 使用 `abstract` 修饰，因此可以声明没有方法体的抽象方法 `eat()`。抽象类也可以定义成员变量、构造方法和具有完整方法体的普通方法，所以这段代码可以正常编译。

  A 错在抽象方法不能有方法体，声明结尾应直接使用分号；B 错在包含抽象方法的类必须声明为抽象类；D 错在 `final` 表示类不能被继承，而 `abstract` 类通常需要由子类继承并补充具体实现，二者作用相互冲突，不能同时修饰同一个类。

- B。执行 `new EmailNotice()` 创建子类对象时，需要先初始化对象中的父类部分，因此会先调用抽象父类 `Notice` 的构造方法，输出 `Notice 构造方法`；随后再执行子类构造方法，输出 `EmailNotice 构造方法`。

  对象创建完成后，`notice.prepare()` 调用父类中已经实现的普通方法，输出 `准备通知内容`。`notice.send()` 调用实例方法时，实际对象是 `EmailNotice`，因此执行子类重写后的 `send()`，输出 `发送邮件通知`。

  A 错在忽略了创建子类对象时必须先执行父类构造方法；C 错在子类构造方法一定在对象创建阶段执行，不会推迟到普通方法调用之后；D 错在抽象类虽然不能直接使用 `new Notice()` 创建对象，但可以定义构造方法，也可以作为引用变量、方法参数或返回值的类型。
