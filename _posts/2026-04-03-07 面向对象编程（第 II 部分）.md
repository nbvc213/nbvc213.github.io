...
>
> 【实操题】请使用 **IDEA** 完成本题。自行创建一个 Java 项目，项目名为 `oop-inheritance-practice`，在 `src` 下创建包 `com.homework.inherit01`，并在该包中创建完成本题所需的类和测试程序。
>
> 现有如下需求：某门店管理系统中，有两类员工：**收银员** 和 **厨师**。它们都有共同的信息，例如姓名和工号，也都需要具备“上班打卡”这一行为；但它们又各自有不同的职责：收银员需要执行“收款”，厨师需要执行“出餐准备”。
>
> 请根据这个需求，使用**继承**完成程序设计，使共同内容抽取到合适的位置，不重复编写；同时保留两类员工各自特有的行为。
>
> 程序运行时需按以下要求输出：
>
> 1. 先输出第一位员工的姓名和工号；
...
>
> ```java
> package com.homework.inherit01;
> 
...
> 
>     public void checkIn() {
>         System.out.println(name + "完成了上班打卡");
>     }
> }
> ```
>
...
>         System.out.println(name + "正在处理顾客收款");
>     }
> }
> ```
>
...
>         System.out.println(name + "正在进行出餐准备");
>     }
> }
> ```
>
...
>         cashier.empId = "C101";
> 
>         System.out.println("姓名：" + cashier.name);
>         System.out.println("工号：" + cashier.empId);
>         cashier.checkIn();
...
>         chef.empId = "K202";
> 
>         System.out.println("姓名：" + chef.name);
>         System.out.println("工号：" + chef.empId);
>         chef.checkIn();
>         chef.prepareMeal();
>     }
> }
...
> ```
>
> 【实操题】请使用 **IDEA** 完成本题。自行创建一个 Java 项目，项目名为 `oop-override-practice`，在 `src` 下创建包 `com.homework.override01`，并在该包中创建完成本题所需的类和测试程序。
>
> 现有如下需求：某内容发布系统中，不同类型的审核员都具备“审核内容”这一行为，但审核规则不同。请设计一个父类来描述审核员的共同行为入口，再设计两个子类分别表示“文本审核员”和“图片审核员”，让它们对“审核内容”这一行为表现出不同的输出效果，以体现**方法覆盖**的作用。
>
> 程序运行时需按以下要求输出：
>
> 1. 先输出第一个审核员的基本信息；
> 2. 然后输出该审核员执行“审核内容”后的提示；
> 3. 再输出第二个审核员的基本信息；
> 4. 最后输出该审核员执行“审核内容”后的提示；
> 5. 两个子类的方法名和参数形式应保持一致，但方法体中的具体实现效果要明显不同。
>
> 【答案】
>
...
>
> ```java
> package com.homework.override01;
> 
...
>         System.out.println(name + "正在审核内容：" + content);
>     }
> }
> ```
>
...
> package com.homework.override01;
> 
> public class TextReviewer extends Reviewer {
>     @Override
>     public void review(String content) {
>         System.out.println(name + "正在进行文本敏感词审核：" + content);
>     }
> }
> ```
>
...
> package com.homework.override01;
> 
> public class ImageReviewer extends Reviewer {
>     @Override
>     public void review(String content) {
>         System.out.println(name + "正在进行图片合规审核：" + content);
>     }
> }
> ```
>
...
>         textReviewer.name = "Tom";
>         textReviewer.reviewerId = "T101";
> 
>         System.out.println("姓名：" + textReviewer.name);
>         System.out.println("工号：" + textReviewer.reviewerId);
>         textReviewer.review("文章：Java 方法覆盖总结");
> 
>         ImageReviewer imageReviewer = new ImageReviewer();
>         imageReviewer.name = "Lucy";
>         imageReviewer.reviewerId = "I202";
> 
>         System.out.println("姓名：" + imageReviewer.name);
>         System.out.println("工号：" + imageReviewer.reviewerId);
>         imageReviewer.review("图片：活动海报");
>     }
> }
> ```
>
...
> 姓名：Tom
> 工号：T101
> Tom正在进行文本敏感词审核：文章：Java 方法覆盖总结
> 姓名：Lucy
> 工号：I202
> Lucy正在进行图片合规审核：图片：活动海报
> ```
